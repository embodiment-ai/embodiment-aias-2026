...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Rotating Speaker Showcase Reel (Continuous Glide & Interactive) ---
  const reelContainer = document.getElementById('speakersReelContainer');
  const reelTrack = document.getElementById('speakersTrack');
  const prevBtn = document.getElementById('reelPrevBtn');
  const nextBtn = document.getElementById('reelNextBtn');

  if (reelContainer && reelTrack) {
    // Clone cards once for a seamless infinite loop
    const cards = Array.from(reelTrack.children);
    cards.forEach(card => {
    ...
    let startX = 0;
    let scrollLeft = 0;
    const glideSpeed = 0.65; // Subtle, gentle continuous glide

    // Arrow controls
    if (prevBtn) {
      prevBtn.addEventListener('click', () => {
        reelContainer.scrollBy({ left: -290, behavior: 'smooth' });
      });
    }
    ...
    reelContainer.addEventListener('mouseenter', () => { isHovered = true; });
    reelContainer.addEventListener('mouseleave', () => { isHovered = false; isDown = false; });
    reelContainer.addEventListener('touchstart', () => { isHovered = true; }, { passive: true });
    reelContainer.addEventListener('touchend', () => { isHovered = false; });

    // Drag to scroll
    reelContainer.addEventListener('mousedown', (e) => {
      isDown = true;
      startX = e.pageX - reelContainer.offsetLeft;
      scrollLeft = reelContainer.scrollLeft;
    });

    reelContainer.addEventListener('mouseup', () => { isDown = false; });
    
    reelContainer.addEventListener('mousemove', (e) => {
      if (!isDown) return;
      e.preventDefault();
      const x = e.pageX - reelContainer.offsetLeft;
      const walk = (x - startX) * 1.5;
      reelContainer.scrollLeft = scrollLeft - walk;
    });

    // Continuous smooth gliding loop
    function glideReel() {
      if (!isHovered && !isDown) {
        reelContainer.scrollLeft += glideSpeed;
        const halfWidth = reelTrack.scrollWidth / 2;
        if (reelContainer.scrollLeft >= halfWidth) {
          reelContainer.scrollLeft -= halfWidth;
        }
      }
      requestAnimationFrame(glideReel);
    }
    requestAnimationFrame(glideReel);
  }
